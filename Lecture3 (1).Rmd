...
So an one-sample t-test is used to determine **whether the mean of a single sample is significantly different from a known or hypothesized population mean.**
...
(1) As the quality control manager, please test the claim of the engineering supervisor that the average axial load is greater than 165 pounds. Use alpha = 0.05 . What assumptions are needed for your test?
H0:average <= 165 or average = 165
Ha:average > 165
...
(b). Please test the research hypothesis at the significance level alpha = 0.05.
...
# When p-value < .05, we will reject H0 _____ data is not normal
# When p-value >= .05, we can say we have normally distributed data
...
Before conducting a one-sample t-test, it's important to **check if the data meets the assumption of normality**, especially if the sample size is small. The t-test relies on the assumption that the data is normally distributed; however, if the sample size is large enough (typically larger than **30**), **the Central Limit Theorem** suggests that the sample mean will approximately follow a normal distribution, even if the data itself is not perfectly normal.

If the data is not normal, we need to use some other methods:
...
If normal or large sample --- t
If not normal and small sample -- wilcox
...
t.test(Before, After, var.equal = F)
# p-value = 0.5707
```
...
```
How to do one/two sample test

1. input the datasets
paired(one sample -- difference) or not(two sample -- independent)
2. check the normality one by one
---> normal (shapio test: p<.05 -- not normal)
...
Please test at alpha = 0.05 whether the new drug is better than the old one.
```{r}
...
data = c(0,10,-3,15,2,27,19,21,18,10,8,-4,7,5,10,11,9,12,7,8)
lab = c(rep('new',10), rep('old',10))
data
lab
wilcox.test(data~lab, alternative = 'greater')
```
...
wilcox.test(diff)
#p=0.03125, which suggests we should reject null hypothesis# #and think there is differences in mean yields for the two varieties of corn#
...
A real data example for t-tests:
The data is about Netflix Original Films & IMDB Scores, which is available online.

```{r}
# Load the data
data <- read.csv('Lecture3_Netflix_data.csv')

# Draw a histogram with density curve for the 'Runtime' variable
library(ggplot2)

ggplot(data, aes(x = Runtime)) +
  geom_histogram(aes(y = ..density..), binwidth = 5, fill = "lightblue", color = "black") +
  geom_density(color = "red") +
  labs(title = "Distribution of Runtime with Density Curve", x = "Runtime", y = "Density") +
  theme_minimal()

# Check normality with Shapiro-Wilk test
shapiro_test_result <- shapiro.test(data$Runtime)
print(shapiro_test_result)
# The sample is not statistically normal
# But the Central Limit Theorem suggests that the sample mean will approximately follow a normal distribution

#584 > 30

# Q-Q plot to visually check normality
qqnorm(data$Runtime)
qqline(data$Runtime, col = "blue", lwd = 2)

# Perform 1-sample t-test comparing Runtime to 91.3
t_test_91_3 <- t.test(data$Runtime, mu = 91.3)
print(t_test_91_3)
# The mean runtime is not equal to 91.3

# Perform 1-sample t-test comparing Runtime to 93
t_test_93 <- t.test(data$Runtime, mu = 93)
print(t_test_93)
# The mean runtime is equal to 93

# Additional visualization: Boxplot of Runtime
ggplot(data, aes(y = Runtime)) +
  geom_boxplot(fill = "lightgreen", color = "black") +
  labs(title = "Boxplot of Runtime", y = "Runtime") +
  theme_minimal()
